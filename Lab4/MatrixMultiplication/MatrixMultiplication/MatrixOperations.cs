...
namespace MatrixMultiplication
{
    public class MatrixOperations
    {
        ...
        {
            for (int i= 0; i < _intermediarTasksList.Row * _intermediarTasksList.Column ; i++)
            {
                _intermediarTasksList.ListElements[i] = new TaskCompletionSource<int>();

            }
            foreach (var x in _intermediarThreadElements)
            {
                Task.Factory.StartNew(() => { MultiplyLineWithColumn(x); });
            }
        }

        public void MultiplyLineWithColumn(Tuple<int, int> _resultMatrixPositions)
        {
